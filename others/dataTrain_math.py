...
class LSTM(nn.Module):
    def __init__(self):
        ...
        # 加入
        self.linear0 = nn.Linear(self.input_size, self.input_size-36)
    ...
    def forward(self, x): 
        ...
        lstm_input = self.linear0(new)
        lstm_input = lstm_input.view(1,96,21)
        h0 = Variable(torch.randn(self.num_layers, lstm_input.size(0), self.hidden_size))
        c0 = Variable(torch.randn(self.num_layers, lstm_input.size(0), self.hidden_size))
        out, _ = self.lstm(lstm_input, (h0, c0))  # 送入一个初始的x值，作为输入以及(h0, c0)
        out = out[:, -1, :]
...
if __name__ == "__main__":
    ...
    # Save the Model
    torch.save(model.state_dict(),name+ 'mathmodel.pkl')